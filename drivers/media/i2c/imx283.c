...
/*
 * TODOs
 *  - Support arbitrary cropping controls
 *  - Reintroduce VFLIP
 *  - Support configurable HOB/VOB
 */
...
#define   IMX283_MDSEL3_VTRIM_EN	BIT(5)
#define IMX283_REG_MDSEL4		CCI_REG8(0x3007)
#define   IMX283_MDSEL4_VTRIM_EN	(BIT(4) | BIT(6))
...
static const struct imx283_mode supported_modes_12bit[] = {
	{
		...
		.height = 3648 + 16, /* Vertical OB added */
	...
	{
		/* Custom 1080p Crop Mode : readout mode 0 */
		.mode = IMX283_MODE_0,
		.bpp = 12,
		.width = 1920, /* Horizontal OB included */
		.height = 1080, /* Vertical OB included */
		.min_hmax = 4500, /* Pixels */
		.min_vmax = 1500, /* Lines */
	...
	{
		/*
		 * 1080p 2/2 Binning Mode :
		 * readout mode 2 with Vertical Positioning.
		 */
		.mode = IMX283_MODE_2A,
		.bpp = 12,
		.width = 1920, /* Horizontal OB included */
		.height = 1080, /* Vertical OB included */
		.min_hmax = 2430, /* Pixels (364 * 480/72 + padding) */
		.min_vmax = 1500, /* Lines */
...
static const char *const imx283_supply_name[] = {
	...
	"vdd1", /* Supply Voltage 2 (1.2V) supply */
	"vdd2", /* Supply Voltage 3 (1.8V) supply */
};
...
static u64 imx283_pixel_rate(struct imx283 *imx283)
{
	...
	u64 link_frequency = link_frequencies[imx283->link_freq_idx];
...
static int imx283_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HBLANK:
		...
		dev_info(imx283->dev, "V4L2_CID_HBLANK : %d  HMAX : %d\n",
	...
	case V4L2_CID_VBLANK:
		imx283->vmax = mode->height + ctrl->val;
		dev_info(imx283->dev, "V4L2_CID_VBLANK : %d  VMAX : %d\n",
...
static int imx283_init_cfg(struct v4l2_subdev *sd,
			   struct v4l2_subdev_state *state)
{
	...
	crop = v4l2_subdev_get_pad_crop(sd, state, IMAGE_PAD);
	*crop = mode->crop;
...
static void imx283_set_framing_limits(struct imx283 *imx283)
{
	...
	dev_info(imx283->dev, "Setting default HBLANK : %lld, VBLANK : %lldn",
		 def_hblank, mode->default_vmax - mode->height);
}
...
static int imx283_standby_cancel(struct imx283 *imx283)
{
	...
	dev_info(imx283->dev, "Using clk freq %d MHz", imx283->freq->mhz / MHZ(1));
				  ...
				  struct v4l2_subdev_state *state)
{
	...
	cci_write(imx283->cci, IMX283_REG_MDSEL3,
		  readout->mdsel3 | IMX283_MDSEL3_VTRIM_EN, &ret);
	cci_write(imx283->cci, IMX283_REG_MDSEL4,
		  readout->mdsel4 | IMX283_MDSEL4_VTRIM_EN, &ret);
	...
	cci_write(imx283->cci, IMX283_REG_Y_OUT_SIZE,
		  mode->height - mode->vertical_ob, &ret);
	cci_write(imx283->cci, IMX283_REG_WRITE_VSIZE, mode->height, &ret);
...
static int imx283_init_controls(struct imx283 *imx283)
{
	...
	imx283->link_freq = v4l2_ctrl_new_int_menu(ctrl_hdlr, &imx283_ctrl_ops,
						   ...
						   imx283->link_freq_idx, link_frequencies);
	if (imx283->link_freq)
