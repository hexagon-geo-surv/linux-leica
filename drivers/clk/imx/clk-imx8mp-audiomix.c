...
#define SAI_PLL_FDIV_CTL0	0x404
#define SAI_PLL_FDIV_CTL1	0x408
#define SAI_PLL_SSCG_CTL	0x40c
#define SAI_PLL_MNIT_CTL	0x410
#define AUDIO_EXT_ADDR		0x500
#define AUDIO_AXI_LIMIT		0x508
...
static struct imx8mp_blk_ctrl_reg {
	const u32	reg;
	u32		val;
} imx8mp_blk_ctrl_regs[] = {
	{ .reg = SAI_MCLK_SEL(0) },
	{ .reg = SAI_MCLK_SEL(1) },
	{ .reg = SAI_MCLK_SEL(2) },
	{ .reg = SAI_MCLK_SEL(3) },
	...
	{ .reg = SAI_MCLK_SEL(5) },
	{ .reg = PDM_SEL },
	{ .reg = SAI_PLL_GNRL_CTL },
	{ .reg = SAI_PLL_FDIV_CTL0 },
	{ .reg = SAI_PLL_FDIV_CTL1 },
	{ .reg = SAI_PLL_SSCG_CTL },
	{ .reg = SAI_PLL_MNIT_CTL },
	{ .reg = AUDIO_EXT_ADDR },
	{ .reg = AUDIO_AXI_LIMIT },
};
...
static int clk_imx8mp_audiomix_probe(struct platform_device *pdev)
{
	...
	err = devm_of_clk_add_hw_provider(&pdev->dev, of_clk_hw_onecell_get,
					   clk_data);
out:
	pm_runtime_put(dev);
...
static int clk_imx8mp_audiomix_suspend(struct device *dev)
{
	struct imx8mp_blk_ctrl *bc = dev_get_drvdata(dev);
	struct imx8mp_blk_ctrl_reg *reg;
	int i;

	for (i = 0; i < ARRAY_SIZE(imx8mp_blk_ctrl_regs); i++) {
		reg = &imx8mp_blk_ctrl_regs[i];
		reg->val = readl(bc->base + reg->reg);
	}

	return 0;
}

static int clk_imx8mp_audiomix_resume(struct device *dev)
{
	struct imx8mp_blk_ctrl *bc = dev_get_drvdata(dev);
	struct imx8mp_blk_ctrl_reg *reg;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(imx8mp_blk_ctrl_regs); i++) {
		reg = &imx8mp_blk_ctrl_regs[i];
		writel(reg->val, bc->base + reg->reg);
	}

	return 0;
}

static DEFINE_RUNTIME_DEV_PM_OPS(clk_imx8mp_audiomix_pm_ops,
				 clk_imx8mp_audiomix_suspend,
				 clk_imx8mp_audiomix_resume, NULL);
